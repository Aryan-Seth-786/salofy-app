function renderBookingConfirmed() {
  ...
  return Shell(`
    <div style="display:flex;flex-direction:column;align-items:center;padding:80px 24px 24px;text-align:center">
      ...
      <div style="font-size:15px;color:${C.text2};margin-bottom:24px">Your appointment has been booked</div>
      ...
      <div style="background:${C.surface2};border-radius:14px;padding:20px;width:100%;border:1px solid ${C.border};text-align:left">
        <div style="display:flex;gap:12px;align-items:center;margin-bottom:16px">
          ...
          <div>
            ...
            <div style="font-size:12px;color:${C.text3}">${s.loc}</div>
          </div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:12px">
          <div>
            <div style="font-size:11px;font-weight:800;color:${C.text3};text-transform:uppercase;letter-spacing:0.1em">Date</div>
            <div style="font-size:15px;font-weight:600;color:${C.text};margin-top:2px">${dateStr}</div>
          </div>
          <div>
            <div style="font-size:11px;font-weight:800;color:${C.text3};text-transform:uppercase;letter-spacing:0.1em">Time</div>
            <div style="font-size:15px;font-weight:600;color:${C.text};margin-top:2px">${timeStr}</div>
          </div>
          <div>
            <div style="font-size:11px;font-weight:800;color:${C.text3};text-transform:uppercase;letter-spacing:0.1em">Total</div>
            <div style="font-size:15px;font-weight:600;color:${C.primary};margin-top:2px">\u20B9${subtotal - discount}</div>
          </div>
        </div>
        <div style="font-size:11px;font-weight:800;color:${C.text3};text-transform:uppercase;letter-spacing:0.1em;margin-bottom:6px">Booked</div>
        ${selPkgs.map(pkgId => {
      ...
      <div style="background:${C.primaryS};border:1px solid ${C.primary}33;border-radius:10px;padding:12px 16px;width:100%;margin-top:12px;display:flex;justify-content:space-between;align-items:center">
        <div>
          <div style="font-size:11px;font-weight:800;color:${C.text3};text-transform:uppercase;letter-spacing:0.1em">Booking Ref</div>
          <div style="font-size:15px;font-weight:700;color:${C.primary};letter-spacing:2px">SF2930</div>
        </div>
        <div style="display:flex;align-items:center;gap:4px;font-size:12px;color:${C.text3};cursor:pointer">${Icons.copy(14, C.text3)} Copy</div>
      </div>

      <button data-action="go-home" class="btn btn--primary" style="margin-top:24px;border-radius:12px">
        Back to Home
      </button>
      <button data-action="view-bookings" style="margin-top:10px;width:100%;padding:14px;background:${C.surface2};color:${C.primary};border:1.5px solid ${C.primary};border-radius:12px;font-family:inherit;font-weight:600;font-size:15px;cursor:pointer">
        View My Bookings
      </button>
    </div>
  `, { activeTab: 'bookings' });
