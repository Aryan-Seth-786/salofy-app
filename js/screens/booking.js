  ...
  return Shell(`
    ...
    <div style="display:flex;gap:8px;padding:0 20px 16px">
      ${['Services', 'Date & Time'].map((label, i) => `
        <div style="flex:1;display:flex;align-items:center;gap:6px">
          <div style="width:24px;height:24px;border-radius:50%;background:${i === 0 ? C.success : C.primary};display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;color:#fff">
            ${i === 0 ? Icons.check(12, '#fff') : '2'}
          </div>
          <span style="font-size:13px;font-weight:${i === 1 ? 600 : 400};color:${i === 1 ? C.text : C.text3}">${label}</span>
        </div>
      `).join('')}
    </div>
    ...
    <div style="padding:0 20px 12px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
        <div style="font-size:13px;font-weight:600;color:${C.text2}">Your selection</div>
        <span data-nav="back" style="font-size:12px;color:${C.primary};font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:3px">
    ...
    <!-- Date picker -->
    <div style="padding:0 20px 12px">
      <div style="font-size:15px;font-weight:600;color:${C.text2};margin-bottom:10px">Select Date</div>
    ...
    <!-- Time picker -->
    <div style="padding:0 20px 12px">
      <div style="font-size:15px;font-weight:600;color:${C.text2};margin-bottom:10px">Select Time</div>
    ...
    <div style="margin:0 20px 8px;padding:14px;background:${C.surface2};border-radius:12px;border:1px solid ${C.border}">
      ...
      <div style="display:flex;justify-content:space-between;font-size:15px;font-weight:700;color:${C.primary};border-top:1px solid ${C.border};padding-top:8px;margin-top:8px">
