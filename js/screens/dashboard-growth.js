function renderDashboardGrowth() {
  ...
  return Shell(`
    <div style="padding:44px 20px 14px;background:#0F0F17;min-height:100%">
      ...
      <div style="display:inline-flex;align-items:center;gap:4px;margin-top:4px;font-size:12px;padding:3px 10px;border-radius:12px;background:rgba(123,104,174,0.15);color:${C.verified}">
      ...
      <div style="margin-top:16px">
        <div style="font-size:15px;font-weight:600;color:var(--ink-50);margin-bottom:12px">Bookings This Month</div>
      ...
      <!-- Top Services -->
      <div style="margin-top:16px">
        <div style="font-size:15px;font-weight:600;color:var(--ink-50);margin-bottom:12px">Most Viewed Services</div>
        ${topServices.map((s, i) => `
          <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:#1A1A28;border:1px solid #363648;border-radius:8px;margin-bottom:8px">
            <div style="display:flex;align-items:center;gap:10px">
              <div style="font-size:15px;font-weight:700;color:#6E6E7A;width:20px">${i + 1}</div>
              <span style="font-size:15px;color:var(--ink-50)">${s.name}</span>
            </div>
            <span style="font-size:13px;color:#B0AEAD;display:flex;align-items:center;gap:4px">${Icons.eye(14, '#6E6E7A')} ${s.views}</span>
          </div>
        `).join('')}
      </div>
    </div>
