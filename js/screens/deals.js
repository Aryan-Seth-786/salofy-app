function renderDeals() {
  ...
  return Shell(`
    <div style="padding:44px 20px 10px">
      ...
      <div style="font-size:13px;color:${C.text3};margin-top:4px">Best deals from salons near you</div>
    </div>

    <!-- Filter Pills -->
    <div style="display:flex;gap:8px;padding:12px 20px;overflow-x:auto" class="hide-sb">
      ${filters.map((f, i) => `
        <div class="pill${i === 0 ? ' pill--primary' : ''}" style="font-size:13px">
    ...
    <div style="padding:0 20px">
      <div style="font-size:11px;font-weight:800;color:${C.text3};text-transform:uppercase;letter-spacing:0.1em;margin-bottom:12px">Salon Deals</div>
      ${deals.map(d => `
        <div style="background:${C.surface};border:1px solid ${C.border};border-radius:10px;overflow:hidden;margin-bottom:12px">
          ...
          <div style="padding:12px">
            <div style="font-size:12px;color:${C.primary};font-weight:500">${d.salon.name}</div>
            <div style="font-size:15px;font-weight:600;color:${C.text};margin-top:2px">${d.title}</div>
            <div style="font-size:12px;color:${C.text3};margin-top:4px">${d.desc}</div>
            <button data-goto-salon="${d.salon.id}" class="btn btn--primary-sm" style="width:100%;margin-top:10px;padding:10px;font-size:13px">Book This Deal</button>
          </div>
        </div>
      `).join('')}
    </div>
