function renderMyBookings() {
  ...
  if (tab === 'Upcoming') {
    content = upcoming.length
      ...
      : `<div style="text-align:center;padding:40px 20px;color:${C.text3};font-size:13px">No upcoming bookings</div>`;

  } else if (tab === 'Completed') {
    content = completed.length
      ? completed.map(cardHtml).join('')
      : `<div style="text-align:center;padding:40px 20px;color:${C.text3};font-size:13px">No completed bookings</div>`;
    ...
    content = `
      ...
      ${shownUpcoming.length
        ...
        : `<div style="padding:12px 0 4px;font-size:12px;color:${C.text3}">No upcoming bookings</div>`}
      ${moreUpcoming > 0 ? `
