function renderSalonProfile() {
  ...
  const servicesPanel = `
    <div class="tab-panel" data-panel="Services" style="${activeTab === 'Services' ? '' : 'display:none'}">
      ...
      <div style="padding:0 20px 4px">
        ${isStarter ? '' : `<div style="font-size:12px;color:${C.text3};margin-bottom:10px">Tap a service to select it for booking</div>`}
  ...
  const staffPanel = s.staff.length > 0 ? `
    <div class="tab-panel" data-panel="Staff" style="${activeTab === 'Staff' ? '' : 'display:none'}">
      <div style="padding:0 20px">
        <div style="font-size:15px;color:${C.text2};margin-bottom:14px">Our professional team</div>
        <div style="display:flex;gap:14px;overflow-x:auto;padding-bottom:8px" class="hide-sb">
          ${s.staff.map(st => `
            <div style="min-width:96px;text-align:center;flex-shrink:0;padding:14px 10px;background:${C.surface2};border-radius:12px;border:1px solid ${C.border}">
              ...
              <div style="font-size:11px;color:${C.text3};margin-top:2px">${st.r}</div>
            </div>
          `).join('')}
        </div>
        <div style="margin-top:16px">
          <div style="font-size:15px;font-weight:600;color:${C.text};margin-bottom:10px">Prefer a specific stylist?</div>
          <div style="background:${C.primaryS};border:1px solid var(--primary-border);border-radius:10px;padding:12px;font-size:13px;color:${C.text2}">
      ...
      <div style="padding:0 20px">
        ...
        <div style="display:flex;align-items:center;gap:16px;padding:12px 0;border-bottom:1px solid ${C.border};margin-bottom:12px">
          <div style="text-align:center">
            ...
            <div style="font-size:12px;color:${C.text3}">${s.reviews} reviews</div>
          </div>
          <div style="flex:1">
            ${[5,4,3,2,1].map(star => {
              const pct = star === 5 ? 72 : star === 4 ? 18 : star === 3 ? 7 : star === 2 ? 2 : 1;
              return `<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">
                <span style="font-size:11px;color:${C.text3};width:8px">${star}</span>
        ...
        <div style="text-align:center;padding:10px 0 4px">
          <span style="font-size:13px;color:${C.primary};font-weight:500;cursor:pointer">See all ${s.reviews} reviews</span>
        </div>
      </div>
    </div>`;
  ...
  const photosPanel = `
    <div class="tab-panel" data-panel="Photos" style="${activeTab === 'Photos' ? '' : 'display:none'}">
      <div style="padding:0 20px">
        <div style="font-size:13px;color:${C.text3};margin-bottom:10px">${s.photos} photos</div>
  ...
  const videoPanel = !isStarter ? `
    <div class="tab-panel" data-panel="Video" style="${activeTab === 'Video' ? '' : 'display:none'}">
      <div style="padding:0 20px">
        <div style="font-size:13px;color:${C.text3};margin-bottom:10px">Salon reels</div>
        ${isPremium ? `
          ...
          <div style="height:180px;background:linear-gradient(135deg,${C.surface2},${C.surface3});border-radius:12px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;border:1px solid ${C.border}">
            ...
            <div style="font-size:13px;color:${C.text3}">Salon Reel</div>
          </div>`}
      </div>
    </div>` : '';
  ...
  return Shell(`
    ...
    <div style="height:240px;background:${heroGrad};position:relative;overflow:hidden">
      ...
      <!-- Photo count pill -->
      <div style="position:absolute;bottom:54px;right:16px;background:rgba(18,15,13,.55);color:#fff;font-size:12px;padding:4px 10px;border-radius:999px;backdrop-filter:blur(4px)">1/${s.photos} photos</div>
    </div>
    ...
    <div style="padding:0 16px;margin-top:-40px;position:relative;z-index:2">
      <div style="background:#fff;border-radius:18px;padding:18px;box-shadow:var(--shadow-md)">
        ...
        <div style="font-size:15px;color:${C.text3};margin-top:4px">${isStarter ? 'Salon' : isPremium ? 'Premium salon' : 'Verified salon'} · ${s.loc}</div>
        ...
        <div style="display:flex;align-items:center;gap:0;margin-top:14px;padding-top:14px;border-top:1px solid ${C.borderS}">
          ...
          <div style="display:flex;align-items:center;gap:5px;flex:1">
            ...
            <span style="font-size:15px;color:${C.ink700}">${s.dist}</span>
          </div>
          <div style="width:1px;height:14px;background:${C.border};margin:0 10px"></div>
          <div style="display:flex;align-items:center;gap:5px;flex:1">
            ${Icons.clock(13, C.success)}
            <span style="font-size:13px;color:${C.success};font-weight:600">${s.hours.split(' - ')[1] ? 'Till ' + s.hours.split(' - ')[1] : s.hours}</span>
          </div>
        </div>
        ${lastVisit ? `
        <div style="display:flex;align-items:center;gap:6px;margin-top:10px">
          ${Icons.calendar(11, C.success)}
          <span style="font-size:12px;color:${C.success};font-weight:500">Visited ${pastVisits.length} time${pastVisits.length > 1 ? 's' : ''} · Last: ${lastVisit.date}</span>
          <span onclick="AppState.bookingsTab='Completed';navigate('my-bookings')"
            style="font-size:12px;color:${C.primary};font-weight:600;cursor:pointer;margin-left:2px">History</span>
        </div>` : ''}
      </div>
    </div>
    ...
    <div style="display:flex;gap:10px;padding:14px 16px 8px">
      <button data-action="book-now" style="flex:1;padding:13px;background:${C.primary};color:#fff;border:none;border-radius:14px;font-family:inherit;font-weight:700;font-size:15px;cursor:pointer;box-shadow:${C.shadowRose};transition:transform 120ms">
    ...
    ${s.deal && !isStarter ? `
      <div style="margin:0 20px 10px;background:${C.successS};border:1px solid var(--success-border);border-radius:10px;padding:10px 14px;display:flex;align-items:center;gap:8px">
        ...
        <div>
          <div style="font-size:15px;font-weight:600;color:${C.success}">${s.deal}</div>
          <div style="font-size:12px;color:${C.text3}">Applied automatically at checkout</div>
        </div>
      </div>` : ''}
    ...
    <div class="salon-summary-bar" style="display:${totalItems > 0 ? 'flex' : 'none'};position:sticky;bottom:0;left:0;right:0;padding:10px 20px 12px;background:${C.bg};border-top:1px solid ${C.border};align-items:center;justify-content:space-between;gap:12px">
      <div>
        <div style="font-size:12px;color:${C.text3}"><span class="ssb-count">${selSvcs.length > 0 && selPkgs.length > 0 ? `${selSvcs.length} service${selSvcs.length > 1 ? 's' : ''} + ${selPkgs.length} package${selPkgs.length > 1 ? 's' : ''}` : selSvcs.length > 0 ? `${selSvcs.length} service${selSvcs.length > 1 ? 's' : ''}` : `${selPkgs.length} package${selPkgs.length > 1 ? 's' : ''}`}</span> selected</div>
        <div style="font-size:16px;font-weight:700;color:${C.primary}"><span class="ssb-price">\u20B9${subtotal}</span>${discount > 0 ? ` <span style="font-size:12px;color:${C.success};font-weight:500">-\u20B9${discount} off</span>` : ''}</div>
      </div>
      <button data-action="book-now" style="padding:12px 22px;background:${C.primary};color:#fff;border:none;border-radius:12px;font-family:inherit;font-weight:700;font-size:15px;cursor:pointer">Book Now</button>
    </div>
  `, { activeTab: 'search' });
}
