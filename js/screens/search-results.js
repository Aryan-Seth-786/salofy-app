  ...
  return Shell(`
    ...
    <div style="display:flex;gap:6px;padding:4px 20px 8px;overflow-x:auto;align-items:center" class="hide-sb">
      ${selSvcs.map(sid => {
        ...
        return `<span style="font-size:12px;padding:5px 10px;background:${C.primaryS};border:1px solid var(--primary-border);border-radius:16px;color:${C.primary};font-weight:500;white-space:nowrap">
          ${svc.label}
        </span>`;
      }).join('')}
      ${[...activeFilters].map(f => `
